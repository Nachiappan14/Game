        ...
        if(this.atomCount() >= this.getThreshold()){
            ...
            if ((p1 >= 0)&&(p2 >=0)&&(p1<Grid.getGrid().l)&&(p2<Grid.getGrid().w)){
                if ((p1+1 >= 0)&&(p2 >=0)&&(p1+1<Grid.getGrid().l)&&(p2<Grid.getGrid().w)){
                    Atom movingAtom = atomsInIt.remove(0);
                    Grid.getGrid().cellList[p1+1][p2].putAtom(movingAtom);
                    controller.get().animateMove(new Pair(p1, p2),new Pair(p1+1, p2),movingAtom);
                }
                if ((p1-1 >= 0)&&(p2 >=0)&&(p1-1<Grid.getGrid().l)&&(p2<Grid.getGrid().w)){
                    Atom movingAtom = atomsInIt.remove(0);
                    Grid.getGrid().cellList[p1-1][p2].putAtom(movingAtom);
                ...
                if ((p1 >= 0)&&(p2-1 >=0)&&(p1<Grid.getGrid().l)&&(p2-1<Grid.getGrid().w)){
                    Atom movingAtom = atomsInIt.remove(0);
                    Grid.getGrid().cellList[p1][p2-1].putAtom(movingAtom);
                    controller.get().animateMove(new Pair(p1, p2),new Pair(p1, p2-1),movingAtom);
                }
                if ((p1 >= 0)&&(p2+1 >=0)&&(p1<Grid.getGrid().l)&&(p2+1<Grid.getGrid().w)){
                    Atom movingAtom = atomsInIt.remove(0);
                    Grid.getGrid().cellList[p1][p2+1].putAtom(movingAtom);
